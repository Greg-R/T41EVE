...
void (*functionPtr[])() = {[]()
                           ...
                           { menuProc.VFOSelect(); },
                           []()
                           { menuProc.ConfigDataOptions(); },
                           []()
                           { menuProc.CalDataOptions(); },
                           []()
                           { menuProc.AGCOptions(); },
                           []()
                           { menuProc.SSBOptions(); },
                           []()
                           { menuProc.EqualizerXmtOptions(); },
                           []()
...
bool parameterAdjustFlag = false; // Used in Button::InputParameterEncoderNoWhile().
...
bool transmitPowerAdjustFlag = false;
bool receiverGainAdjustFlag = false;
...
FLASHMEM void InitializeDataArrays()
{
  ...
  int LP_F_help;
// DB2OO, 11-SEP-23: don't use the fixed sizes, but use the calculated ones, otherwise a code change will create very difficult to find problems
// Get rid of this macro.  Use for loops to zero out the arrays.  Make sure the array sizes are correct.
...
displayUpdateCounter = 0;
  ...
  if (bands.bands[ConfigData.currentBand].mode == RadioMode::CW_MODE and ConfigData.keyType == 1 and (digitalRead(ConfigData.paddleDit) == HIGH) and (digitalRead(ConfigData.paddleDah) == HIGH))
    radioState = RadioState::CW_RECEIVE_STATE;
  if (bands.bands[ConfigData.currentBand].mode == RadioMode::CW_MODE and ConfigData.keyType == 0 and digitalRead(KEYER_DIT_INPUT_TIP) == HIGH)
    radioState = RadioState::CW_RECEIVE_STATE;
  if (bands.bands[ConfigData.currentBand].mode == RadioMode::CW_MODE and keyPressedOn == true and ConfigData.keyType == 0)
    radioState = RadioState::CW_TRANSMIT_STRAIGHT_STATE;
  if (bands.bands[ConfigData.currentBand].mode == RadioMode::CW_MODE and keyPressedOn == true and ConfigData.keyType == 1)
  ...
  */
  ...
  if (lastState != radioState)
  {
    ...
    if (radioState == RadioState::SSB_RECEIVE_STATE and lastState == RadioState::SSB_TRANSMIT_STATE)
      SetAudioOperatingState(radioState);
    if (radioState == RadioState::SSB_TRANSMIT_STATE and lastState == RadioState::SSB_RECEIVE_STATE)
      SetAudioOperatingState(radioState);

    if (radioState == RadioState::FT8_RECEIVE_STATE and lastState == RadioState::FT8_TRANSMIT_STATE)
      SetAudioOperatingState(radioState);
    if (radioState == RadioState::FT8_TRANSMIT_STATE and lastState == RadioState::FT8_RECEIVE_STATE)
      SetAudioOperatingState(radioState);

    if (radioState == RadioState::CW_RECEIVE_STATE and lastState == RadioState::CW_TRANSMIT_STRAIGHT_STATE)
      SetAudioOperatingState(radioState);
    ...
    if (radioState == RadioState::SAM_RECEIVE_STATE and lastState != RadioState::SAM_RECEIVE_STATE)
      SetAudioOperatingState(radioState);
    if (radioState == RadioState::AM_RECEIVE_STATE and lastState != RadioState::AM_RECEIVE_STATE)
      SetAudioOperatingState(radioState);

    // Customization dependent on required mode.
    if (radioState == RadioState::CW_RECEIVE_STATE)
      modecontrol.CWReceiveMode();
    if (radioState == RadioState::CW_TRANSMIT_STRAIGHT_STATE)
      modecontrol.CWTransmitMode();
    if (radioState == RadioState::CW_TRANSMIT_KEYER_STATE)
      modecontrol.CWTransmitMode();

    if (radioState == RadioState::SSB_RECEIVE_STATE)
      modecontrol.SSBReceiveMode();
    if (radioState == RadioState::SSB_TRANSMIT_STATE)
      modecontrol.SSBTransmitMode();

    if (radioState == RadioState::FT8_RECEIVE_STATE)
      modecontrol.FT8ReceiveMode();
    if (radioState == RadioState::FT8_TRANSMIT_STATE)
      modecontrol.FT8TransmitMode();

    if (radioState == RadioState::AM_RECEIVE_STATE)
      modecontrol.AMReceiveMode();
    if (radioState == RadioState::SAM_RECEIVE_STATE)
      modecontrol.SAMReceiveMode();
  }
...
//    initPowerCoefficients();
      ...
      {
        if (digitalRead(KEYER_DIT_INPUT_TIP) == HIGH)
        { // Turn off CW signal
          keyPressedOn = false;
          straightKeyStart = false;
          if (cwKeyDown)
          { // Initiate falling CW signal.
            ...
            cwexciter.CW_ExciterIQData(CW_SHAPING_ZERO); //  No waveforms; but DC offset is still present.
        }
      }
...
//        Serial.printf("evenmenucontrol.subMenuChoice = %d\n", menuProc.subMenuChoice);
  ...
  if (ms_500.check() == 1)
  { // For clock updates AFP 10-26-22
